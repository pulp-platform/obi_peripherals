...
module obi_uart_register import obi_uart_pkg::*; #(
  ...
  always_comb begin : hw_update
    ...
    new_reg.LSR.fifo_err   = reg_write_i.rx.fifo_err_valid ?
                             reg_write_i.rx.fifo_err : lsr_q.fifo_err;
    new_reg.LSR.tx_empty   = reg_write_i.tx.empty_valid ?
                             reg_write_i.tx.tx_empty : lsr_q.tx_empty;
    new_reg.LSR.thr_empty  = reg_write_i.tx.thr_valid ?
                             reg_write_i.tx.thr_empty : lsr_q.thr_empty;
    new_reg.LSR.break_ind  = reg_write_i.rx.break_valid ?
                             reg_write_i.rx.break_ind : lsr_q.break_ind;
    new_reg.LSR.frame_err  = reg_write_i.rx.frame_valid ?
                             reg_write_i.rx.frame_err : lsr_q.frame_err;
    new_reg.LSR.par_err    = reg_write_i.rx.par_valid      ? reg_write_i.rx.par_err : lsr_q.par_err;
    new_reg.LSR.data_ready = reg_write_i.rx.dr_valid ?
                             reg_write_i.rx.data_ready : lsr_q.data_ready;
  ...
  always_comb begin : read_strobe_comb
    obi_read_rhr = 1'b0;
    obi_read_isr = 1'b0;
    obi_read_lsr = 1'b0;
    ...
    if (req_q && !we_q && addr_valid_q && !lcr_q.dlab) begin
      unique case (word_addr_q)
        ...
        RegAddrLSR: obi_read_lsr = 1'b1;
        default: ;
      endcase
    end
  end

  always_comb begin : write_strobe_comb
    obi_write_thr  = 1'b0;
    obi_write_dllm = 1'b0;
    ...
    if (obi_req_i.req && obi_req_i.a.we && obi_req_i.a.be[0] && addr_valid_d) begin
      if (!lcr_q.dlab) begin
        ...
        obi_write_dllm = word_addr_d inside {RegAddrDLL, RegAddrDLM};
      end
    end
  end
